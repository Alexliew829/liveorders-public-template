...
export default async function handler(req, res) {
  ...
  try {
    // ✅ 抓最新 Facebook 贴文 ID（feed 或 video）
    ...
    // ✅ 这时才写入 config.post_id（避免一开始就阻止商品写入）
    ...
    return res.status(500).json({ error: '执行失败', details: err.m
