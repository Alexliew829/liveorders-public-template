  ...
  try {
    ...
    for (const comment of comments) {
      ...
      if (!message || !from || from.id !== PAGE_ID) {
        skippedReasons.push({ id: comment_id, reason: '无 message 或 非主页留言' });
        continue;
      }

      const match = message.match(/\b([AB])[ \-_.～]*0*(\d{1,3})\b/i);
      if (!match) {
        skippedReasons.push({ id: comment_id, message, reason: '无法识别商品编号' });
        continue;
      }
      ...
      if (!priceMatch) {
        skippedReasons.push({ id: comment_id, message, reason: '无法识别价格' });
        continue;
      }
    ...
    return res.status(200).json({
      ...
      isForce,
      used: 'feed',
      skippedDetails: skippedReasons.slice(0, 15),
    });
